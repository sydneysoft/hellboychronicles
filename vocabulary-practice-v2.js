...
async function definition(entry){
 const key=`${source}:${comparable(entry.lemma||entry.word,sourceLocale)}:${comparable(entry.translation,targetLocale)}`;
 if(definitions[key])return definitions[key];
 try{
  const englishTerm=source==='en'?(entry.lemma||entry.word):firstAnswer(entry.translation);
...
async function loadCurrent(){
 ...
 q('.practice-v2-cue').textContent=String(current.word).toLocaleUpperCase(sourceLocale);
 q('.practice-language').textContent=`${sourceName.toUpperCase()} → ${targetName.toUpperCase()}`;
 q('.practice-v2-definition').textContent=ui.loading;
 q('#practiceV2Word').value='';q('#practiceV2Sentence').value='';q('.practice-v2-word-feedback').innerHTML='';q('.practice-v2-sentence-feedback').innerHTML='';q('.practice-v2-sentence-step').hidden=true;q('.practice-v2-next').hidden=true;q('.practice-v2-check-word').disabled=false;q('#practiceV2Word').disabled=false;q('.practice-v2-answer').textContent=firstAnswer(current.translation);
 const value=await definition(current);if(current===queue[index])q('.practice-v2-definition').textContent=value;
}

function checkWord(){
 const typed=q('#practiceV2Word').value.trim(),given=comparable(typed),answers=splitAnswers(current.translation).map(answer=>comparable(answer)),feedback=q('.practice-v2-word-feedback');
 ...
 const expected=firstAnswer(current.translation);let first=0;while(first<typed.length&&first<expected.length&&comparable(typed[first])===comparable(expected[first]))first++;
...
function localSentenceErrors(value){
 const errors=[],sentence=value.trim(),lower=comparable(sentence),forms=splitAnswers(current.translation).map(answer=>comparable(answer));
 if(!forms.some(form=>form&&lower.includes(form)))errors.push(ui.useWord);
 if(sentence.split(/\s+/u).filter(Boolean).length<3)errors.push(ui.short);
 if(!(target==='uk'?/^[А-ЯІЇЄҐ]/u:/^[A-Z]/u).test(sentence))errors.push(ui.capital);
...
async function grammarErrors(value){
 const body=new URLSearchParams({text:value,language:target==='uk'?'uk-UA':'en-US'}),response=await fetch('https://api.languagetool.org/v2/check',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});
...
async function checkSentence(){
 ...
 stats[comparable(current.lemma||current.word,sourceLocale)]=true;write(statsKey,stats);feedback.innerHTML=`<p class="practice-success">✓ ${ui.well}</p>`;q('.practice-v2-next').hidden=queue.length<2;q('.practice-v2-progress').textContent=`${ui.word} ${index+1} / ${queue.length} · ${Object.values(stats).filter(Boolean).length} ${ui.learned}`;
}

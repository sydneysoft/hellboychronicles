(()=>{
  ...
  if(!root)return;
  const names={en:'ENGLISH',uk:'UKRAINIAN',pl:'POLISH',fr:'FRENCH',de:'GERMAN',es:'SPANISH'};
  const locales={en:'en-US',uk:'uk-UA',pl:'pl-PL',fr:'fr-FR',de:'de-DE',es:'es-ES'};
  const cache=new Map();
  const fixes={
    'en:uk:city':'місто',
    'en:uk:cities':'міста',
  ...
  async function translate(text,source,target){
    ...
    const fix=fixes[`${source}:${target}:${normalized}`];
    if(fix)return fix;
    const k=`${source}:${target}:${normalized}`;
    if(cache.has(k))return cache.get(k);
    const r=await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(text)}&langpair=${source}|${target}`,{headers:{Accept:'application/json'}});
    if(!r.ok)throw 0;
    const j=await r.json();
    const value=j?.responseData?.translatedText?.trim();
    if(!value)throw 0;
    cache.set(k,value);
    return value
  }
  ...
  async function show(){clearTimeout(timer);const sel=getSelection();if(!sel||sel.isCollapsed||!sel.rangeCount)return;const a=sel.anchorNode,f=sel.focusNode;if(!root.contains(a)||!root.contains(f))return;const word=clean(sel.toString());if(!word||word.length>80||word.split(/\s+/).length>5)return;const rect=sel.getRangeAt(0).getBoundingClientRect();if(!rect.width&&!rect.height)return;const source=currentLang();close();pop=document.createElement('div');pop.className='ucv-pop';pop.innerHTML=`<div class="ucv-top"><div><div class="ucv-word"></div><div class="ucv-label"></div></div><button class="ucv-close" aria-label="Close">×</button></div><div class="ucv-translation ucv-loading">Translating…</div><div class="ucv-actions"><button class="ucv-save">SAVE WORD</button><button class="ucv-listen">🔊 LISTEN</button><button class="ucv-practice-open">✎ PRACTICE WORD</button></div><div class="ucv-practice"></div>`;pop.querySelector('.ucv-word').textContent=word;pop.querySelector('.ucv-label').textContent=source==='en'?'ENGLISH → ES / DE / FR / PL / UA':`${names[source]} → ENGLISH`;document.body.appendChild(pop);place(rect);pop.querySelector('.ucv-close').onclick=close;pop.querySelector('.ucv-listen').onclick=()=>speak(word,source);
    try{
      let primary='',target='en';
      if(source==='en'){
        const [es,de,fr,pl,uk]=await Promise.all([translate(word,'en','es'),translate(word,'en','de'),translate(word,'en','fr'),translate(word,'en','pl'),translate(word,'en','uk')]);
        primary=es;target='es';
        if(!pop)return;
        pop.querySelector('.ucv-translation').innerHTML=`<div class="ucv-other"><div><b>🇪🇸 Spanish:</b> ${es}</div><div><b>🇩🇪 German:</b> ${de}</div><div><b>🇫🇷 French:</b> ${fr}</div><div><b>🇵🇱 Polish:</b> ${pl}</div><div><b>🇺🇦 Ukrainian:</b> ${uk}</div></div>`;
      }else{

(()=>{
  ...
  if(!root)return;
  const names={en:'ENGLISH',uk:'UKRAINIAN',pl:'POLISH',fr:'FRENCH',de:'GERMAN'};
  const locales={en:'en-US',uk:'uk-UA',pl:'pl-PL',fr:'fr-FR',de:'de-DE'};
  const cache=new Map();
  ...
  async function show(){clearTimeout(timer);const sel=getSelection();if(!sel||sel.isCollapsed||!sel.rangeCount)return;const a=sel.anchorNode,f=sel.focusNode;if(!root.contains(a)||!root.contains(f))return;const word=clean(sel.toString());if(!word||word.length>80||word.split(/\s+/).length>5)return;const rect=sel.getRangeAt(0).getBoundingClientRect();if(!rect.width&&!rect.height)return;const source=currentLang();close();pop=document.createElement('div');pop.className='ucv-pop';pop.innerHTML=`<div class="ucv-top"><div><div class="ucv-word"></div><div class="ucv-label"></div></div><button class="ucv-close" aria-label="Close">×</button></div><div class="ucv-translation ucv-loading">Translating…</div><div class="ucv-actions"><button class="ucv-save">SAVE WORD</button><button class="ucv-listen">🔊 LISTEN</button><button class="ucv-practice-open">✎ PRACTICE WORD</button></div><div class="ucv-practice"></div>`;pop.querySelector('.ucv-word').textContent=word;pop.querySelector('.ucv-label').textContent=source==='en'?'ENGLISH → DE / FR / PL / UA':`${names[source]} → ENGLISH`;document.body.appendChild(pop);place(rect);pop.querySelector('.ucv-close').onclick=close;pop.querySelector('.ucv-listen').onclick=()=>speak(word,source);
    try{
      let primary='',target='en';
      if(source==='en'){
        const [de,fr,pl,uk]=await Promise.all([translate(word,'en','de'),translate(word,'en','fr'),translate(word,'en','pl'),translate(word,'en','uk')]);
        primary=de;target='de';
        if(!pop)return;
        pop.querySelector('.ucv-translation').innerHTML=`<div class="ucv-other"><div><b>🇩🇪 German:</b> ${de}</div><div><b>🇫🇷 French:</b> ${fr}</div><div><b>🇵🇱 Polish:</b> ${pl}</div><div><b>🇺🇦 Ukrainian:</b> ${uk}</div></div>`;
      }else{

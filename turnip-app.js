...
for(let page=1;page<=totalPages;page++){
  ...
  const figure=document.createElement('figure');figure.className='comic-page';figure.dataset.page=page;
...
const goTo=page=>{const target=Math.max(1,Math.min(totalPages,page));activateReader(target);figures[target-1].scrollIntoView({behavior:'smooth',block:'start'})};
const update=page=>{currentPage=page;if(readerActive)warmPage(page);document.getElementById('progressText').textContent=`${language==='uk'?'СТОРІНКА':'PAGE'} ${page} ${language==='uk'?'З':'OF'} ${totalPages}`;document.getElementById('controlPage').textContent=`${page} / ${totalPages}`;document.getElementById('progressBar').style.width=`${page/totalPages*100}%`;history.replaceState(null,'',`#page-${page}`)};
...
const dialog=document.getElementById('lightbox'),image=document.getElementById('lightboxImage');figures.forEach(f=>f.querySelector('img').onclick=e=>{activateReader(Number(f.dataset.page));const target=e.currentTarget;if(target.dataset.src){target.src=target.dataset.src;target.removeAttribute('data-src')}image.src=target.src;dialog.showModal()});document.getElementById('closeLightbox').onclick=()=>dialog.close();dialog.onclick=e=>{if(e.target===dialog)dialog.close()};
...
const initial=location.hash.match(/page-(\d+)/);if(initial){const page=Number(initial[1]);activateReader(page);setTimeout(()=>goTo(page),120)}
